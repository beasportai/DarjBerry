  ...
  return (
    <div className="w-full p-4 space-y-8">
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-xl font-bold mb-4">Blueberry Farming Financial Analysis: Traditional vs. Imported Plants</h2>
              ...
              <tbody>
                {keyMetrics.map((metric, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                    ...
                    <td className="border p-2 text-right">
                      {metric.name.includes("Period") ? 
                        `${(metric.original - metric.imported).toFixed(2)} years` :
                        metric.name.includes("ROI") ?
                          `${(metric.imported - metric.original).toFixed(2)}%` :
                          `₹${((metric.imported - metric.original) / 100).toFixed(2)} Cr`
                      }
                    </td>
                  </tr>
                ))}
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={yearlyData}>
                ...
                <YAxis label={{ value: 'Production (tonnes)', angle: -90, position: 'insideLeft' }} />
                <Tooltip formatter={(value) => `${value} tonnes`} />
                <Legend />
                <Bar dataKey="originalProduction" name="Traditional Setup" fill="#8884d8" />
                <Bar dataKey="importedProduction" name="Imported Plants" fill="#82ca9d" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={yearlyData}>
                ...
                <Legend />
                <Bar dataKey="originalProfit" name="Traditional Setup" fill="#8884d8" />
                <Bar dataKey="importedProfit" name="Imported Plants" fill="#82ca9d" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={yearlyData}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="year" label={{ value: 'Year', position: 'insideBottom', offset: -5 }} />
              <YAxis label={{ value: 'Cash Flow (₹ Lakhs)', angle: -90, position: 'insideLeft' }} />
              <Tooltip formatter={(value) => `₹${value} Lakhs`} />
              <Legend />
              <Line type="monotone" dataKey="originalCumulativeCashFlow" name="Traditional Setup" stroke="#8884d8" strokeWidth={2} />
              <Line type="monotone" dataKey="importedCumulativeCashFlow" name="Imported Plants" stroke="#82ca9d" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h3 className="text-lg font-semibold mb-2">Financial Analysis Summary</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li>The imported plants model requires a higher initial investment (₹14.06 Cr vs ₹8.36 Cr) but delivers significantly accelerated returns.</li>
          <li>Payback period is reduced by 0.36 years (approximately 4.3 months) with imported plants.</li>
          <li>By Year 5, the imported plants model generates 52% higher annual profits (₹26.05 Cr vs ₹17.14 Cr).</li>
          <li>The total 5-year profit advantage is ₹42.49 Cr with the imported plants model.</li>
          <li>Production stabilizes at 270 tonnes annually with imported plants vs 180 tonnes with traditional setup.</li>
          <li>The imported plants model reaches full production capacity by Year 3, while the traditional model reaches it in Year 4.</li>
        </ul>
      </div>
    </div>
  );
